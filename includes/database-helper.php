...
class SongDB{
    ...
    private static $sqlSearch = "SELECT title, artist_name as name, type_name as type, genre_name as genre, year, duration, bpm, energy, danceability, liveness, valence, acousticness, speechiness, popularity, songs.artist_id, artist_type_id, songs.genre_id, song_id
    FROM songs
    ...
    public function searchByYearGreater($greater){
        $sql = self::$sqlSearch." WHERE year >= $greater";
        $results = DatabaseHelper::runQuery($this->pdo, $sql);
        return $results -> fetchAll();
    }
    ...
    public function searchByPopularityGreater($greater){
        $sql = self::$sqlSearch ." WHERE popularity >= $greater";
        $results = DatabaseHelper::runQuery($this->pdo, $sql);
        return $results -> fetchAll();
    }
    ...
    public function getTopPopularSongs(){
        $sql = "SELECT popularity, artist_name as name, title, songs.artist_id
        FROM songs
        INNER JOIN artists
    ...
    public function getTopOneHitsSongs(){
        $sql = "SELECT artist_name as name, title, COUNT(title) as numSongs, popularity, songs.artist_id
        FROM songs
        INNER JOIN artists
        ON artists.artist_id = songs.artist_id
    ...
    public function getTopAcousticSongs(){
        $sql = "SELECT title, acousticness, duration
        FROM songs
        GROUP BY title 
...
class ArtistDB{
    ...
    public function getTopArtists(){
        $sql = "SELECT artist_name as name, COUNT(title) as numSongs, songs.artist_id
        FROM artists
        INNER JOIN songs
